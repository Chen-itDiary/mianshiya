...
import com.xiaochen.mianshiya.mapper.QuestionBankQuestionMapper;
...
//        List<Long> validQuestionIdList = questionList.stream().map(Question::getId).collect(Collectors.toList());
        ...
        // 直接生成合法的题目id列表
        List<Long> validQuestionIdList = questionService.listObjs(wrapper, object -> (Long) object);
        ThrowUtils.throwIf(CollUtil.isEmpty(validQuestionIdList), ErrorCode.NOT_FOUND_ERROR, "合法题目列表不存在");
        // 查询 id 列表中是否已经有插入题库之中，避免重复插入
        LambdaQueryWrapper<QuestionBankQuestion> lambdaQueryWrapper = Wrappers.lambdaQuery(QuestionBankQuestion.class).eq(QuestionBankQuestion::getQuestionBankId, questionBankId)
...
//                questionBankQuestion.setQuestionId(subList.get(j));
//                questionBankQuestion.setUserId(loginUser.getId());
//                questionBankQuestions.add(questionBankQuestion);
//            }
            // stream流写法
            List<QuestionBankQuestion> questionBankQuestions = subList.stream().map(id -> {
                QuestionBankQuestion questionBankQuestion = new QuestionBankQuestion();
                questionBankQuestion.setQuestionBankId(questionBankId);
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public Integer batchRemoveQuestionsFromBank(List<Long> questionIdLIst, Long questionBankId) {
        //参数校验
        ThrowUtils.throwIf(CollUtil.isEmpty(questionIdLIst), ErrorCode.PARAMS_ERROR);
        ThrowUtils.throwIf(questionBankId == null, ErrorCode.PARAMS_ERROR);
        //检查题库是否存在
        ...
        // 执行删除（分批次插入数据---事务）
        int batchSize = 1000;
        int totalQuestionSize = existQuestionList.size();
        AtomicInteger successDelet = new AtomicInteger();
        for(int i = 0; i < totalQuestionSize; i += batchSize){
            List<Long> subList = existQuestionList.subList(i, Math.min(i + batchSize, totalQuestionSize));
            // 使用事务分批处理数据
            // 获取代理
            QuestionBankQuestionServiceImpl questionBankQuestionService = (QuestionBankQuestionServiceImpl) AopContext.currentProxy();
            CompletableFuture<Void> voidCompletableFuture = CompletableFuture.runAsync(() -> {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public Integer batchRemoveQuestionsToBankInner(List<Long> questionBankQuestions, Long questionBankId) {
        int countDelete = 0;
        try {
        LambdaQueryWrapper<QuestionBankQuestion> lambdaQueryWrapper = Wrappers.lambdaQuery(QuestionBankQuestion.class)
                    .eq(QuestionBankQuestion::getQuestionBankId, questionBankId)
                    .in(QuestionBankQuestion::getQuestionId, questionBankQuestions);
            countDelete = this.baseMapper.delete(lambdaQueryWrapper);
            ThrowUtils.throwIf(countDelete == 0, ErrorCode.OPERATION_ERROR, "向题库删除题目失败");
        }catch (DataAccessException e) {
            log.error("数据库连接问题、事务问题等导致操作失败, 错误信息: {}", e.getMessage());
            throw new BusinessException(ErrorCode.OPERATION_ERROR, "数据库操作失败");
        } catch (Exception e) {
